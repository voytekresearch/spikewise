"""
config.py
---------
Experiment-wide constants and per-cell metadata for the spe-1 dataset.

Sections:
  - Priority cells (target cells for LFP analysis)
  - Hardware/experiment parameters
  - Data and pickle root paths (override via environment variables)
  - Per-cell sampling rates, channel predictions, spike thresholds
  - Per-cell metadata: patch type, cell type, cortical depth,
    dark neuron flag, EAP waveform quality
"""
import os

# =============================================================================
# ANALYSIS TARGETS
# =============================================================================

# Cells with strongest spike waveform cluster differences (highest mean nRMSE across features)
# Updated from pop_spk_waveform_cluster_comparisons.ipynb → Section G
PRIORITY_CELLS = [18, 21, 22, 24, 26, 28, 45]

# Top cells by nRMSE that also have low temporal drift (|mean rho| < 0.2)
# These provide a contrast group: large waveform differences not confounded by time-in-recording
# Updated from pop_spk_waveform_cluster_comparisons.ipynb → Section G
HIGH_DIFF_LOW_DRIFT_CELLS = [15, 26, 46]

# =============================================================================
# HARDWARE / EXPERIMENT PARAMETERS
# =============================================================================

# Number of channels on the Neuropixels probe
NPX_CHANNELS = 384
CELL_IDS = ["c1", "c2", "c3", "c4", "c5", "c6", "c7", "c8", "c10", "c12", "c13", "c14", "c15", "c16","c17", "c18", "c19", "c20", 
            "c21", "c22", "c23", "c24", "c25", "c26", "c27", "c28", "c29",
            "c31", "c32", "c33", "c34", "c35", "c36", "c37", "c38",
            "c39", "c40", "c41","c42","c43", "c44", "c45", "c46"]

# =============================================================================
# DATA AND PICKLE PATHS
# =============================================================================
# These default to the current machine's paths but can be overridden without
# editing this file by setting environment variables before running:
#
#   export SPE1_DATA_ROOT="/path/to/Neuropixel Paired Recordings/Recordings"
#   export SPE1_PICKLE_ROOT="/path/to/spe1_pickles"
#
SPE1_DATA_ROOT = os.environ.get(
    "SPE1_DATA_ROOT",
    "/Users/blancamartin/Desktop/Voytek_Lab/spike_waveform/Neuropixel Paired Recordings/Recordings",
)
SPE1_PICKLE_ROOT = os.environ.get(
    "SPE1_PICKLE_ROOT",
    "/Users/blancamartin/Desktop/Voytek_Lab/spike_waveform/spe1_pickles",
)

# =============================================================================
# SAMPLING RATES (Hz)
# =============================================================================
NPX_FS = 30000   # Neuropixels probe raw sampling rate
LFP_FS = 2500    # Neuropixels LFP downsampled sampling rate

# =============================================================================
# FILTER PARAMETERS  (Hz)
# =============================================================================
FILTER_SETTINGS = {
    "lfp": [0.1, 250],
    "patch": [10, 25000],
    "npx": [300, 14000]
}

# =============================================================================
# PER-CELL METADATA
# =============================================================================

# Patch clamp sampling frequency (Hz) per cell
DICT_PATCH_FS = {
    1:50023.91875795,
    2: 50023.91258241,
    3: 50023.91669944,
    4: 50023.91125908,
    5: 50023.90743614,
    6: 50023.89817284,
    7: 50023.91361167,
    8: 50023.90949465,
    10: 50023.90023135,
    12: 50023.89817284,
    13: 50023.89817284,
    14: 50023.89868747,
    15: 50023.89405582,
    16: 50023.90270156,
    17: 50023.90023135,
    18: 50023.89199731,
    19: 50023.88376327,
    20: 50023.87964625,
    21: 50023.89714358,
    22: 50023.89474199,
    23: 50023.89199731,
    24: 50023.89405582,
    25: 50023.88942417,
    26: 50023.88925263,
    27: 50023.88788029,
    28: 50023.87964625,
    29: 50023.90640688,
    31: 50023.89817284,
    32: 50023.89611433,
    33: 50023.89611433,
    34: 50023.88170476,
    35: 50023.88788029,
    36: 50023.88376327, 
    37: 50023.88788029,
    38: 50023.86935371,
    39: 50023.88170476,
    40: 50023.87347073, 
    41: 50023.88170476,
    42: 50023.88170476,
    43:50023.87552924,
    44: 50023.87244147,
    45: 50023.87347073,
    46: 50023.87552924
}

# Best Neuropixels channel prediction per cell (used to extract single-channel LFP/NPX)
DICT_CHAN_PRED = {


1:204,
2:219,
3:188,
4:204,
5:251,
6:172,
7:188,
8:188,
10:219,
12:138,
13:138,
14:159,
15:183,
16:220,
17:279,
18:157,
19:178,
20:199,
21:180,
22:205,
23:227,
24:248,
25:259,
26:261,
27:223,
28:234,
29:203,
31:135,
32:153,
33:160,
34:144,
35:160,
36:141,
37:197,
38:105,
39:114,
40:148,
41:95,
42:108,
43:147,
44:178,
45:178,
46:186


}

# Spike detection amplitude threshold (mV) per cell
DICT_SPK_THRESH = {
1:1.25,
2:62.2,
3:0.81,
4:2.8,
5:10.4,
6:85,
7:10.5,
8:3.41,
10:65.1,
12:75,
13:81.3,
14:137.2,
15:62.6,
16:126.3,
17:55.2,
18:24.9,
19:105,
20:27.23,
21:5.2,
22:67.5,
23:1.7,
24:56.1,
25:95.4,
26:43.9,
27:5.9,
28:40.6,
29:80.3,
31:77.5,
32:53.7,
33:1.7,
34:1.24,
35:70.1,
36:117.8,
37:47.4,
38:81,
39:47,
40:3.86,
41:37.2,
42:2.06,
43:53,
44:50.1,
45:64.1,
46:76.9
}


# Recording modality and clamp mode per cell: "Juxta" or "WC" (whole-cell), "IC" or "VC"
DICT_PATCH_TYPE = {1:"Juxta, IC",
2:"Juxta, VC",
3:"Juxta, IC",
4: "WC, IC",
5:"WC, IC",
6:"Juxta, VC",
7:"Juxta, IC",
8:"Juxta, IC",
10:"Juxta, VC",
12:"Juxta, VC",
13:"Juxta, VC",
14:"Juxta, VC",
15:"Juxta, VC",
16:"Juxta, VC",
17:"Juxta, VC",
18:"Juxta, VC",
19:"Juxta, VC",
20:"Juxta, VC",
21:"Juxta, IC",
22:"Juxta, VC",
23:"Juxta, IC",
24:"Juxta, VC",
25:"Juxta, VC",
26:"Juxta, VC",
27:"WC, IC",
28:"Juxta, VC",
29:"Juxta, VC",
31:"Juxta, VC",
32:"Juxta, VC",
33:"Juxta, IC",
34:"Juxta, IC",
35:"Juxta, VC",
36:"Juxta, VC",
37:"Juxta, VC",
38:"Juxta, VC",
39:"Juxta, VC",
40:"WC, IC",
41:"Juxta, VC",
42:"WC, IC",
43:"Juxta, VC",
44:"Juxta, VC",
45:"Juxta, VC",
46:"Juxta, VC"}

# Cell type per cell: "PC" (pyramidal) or "IN" (interneuron)
DICT_CELL_TYPE = {
1:"PC",
2:"PC",
3:"PC",
4:"PC",
5:"PC",
6:"PC",
7:"IN",
8:"IN",
10:"PC",
12:"IN",
13:"PC",
14:"PC",
15:"PC",
16:"IN",
17:"PC",
18:"IN",
19:"PC",
20:"PC",
21:"PC",
22:"IN",
23:"PC",
24:"PC",
25:"PC",
26:"PC",
27:"PC",
28:"PC",
29:"PC",
31:"PC",
32:"PC",
33:"PC",
34:"PC",
35:"PC",
36:"PC",
37:"PC",
38:"PC",
39:"PC",
40:"PC",
41:"PC",
42:"PC",
43:"PC",
44:"PC",
45:"PC",
46:"PC"
}


# Whether the extracellular AP (EAP) waveform is clearly visible on the Neuropixels channel
DICT_EAP_WAV = {
1:False,
2:True,
3:False,
4:True,
5:False,
6:False,
7:True,
8:False,
10:False,
12:True,
13:False,
14:True,
15:True,
16:True,
17:False,
18:True,
19:True,
20:False,
21:True,
22:True,
23:False,
24:True,
25:False,
26:True,
27:True,
28:True,
29:True,
31:False,
32:False,
33:False,
34:False,
35:False,
36:False,
37:True,
38:False,
39:False,
40:False,
41:False,
42:True,
43:False,
44:True,
45:True,
46:True
}


# Whether the cell is a "dark neuron" (not detected by Kilosort / no clear EAP on NPX)
DICT_DARK_NEURONS = {

1:False,
2:True,
3:True,
4:False,
5:False,
6:False,
7:False,
8:False,
10:False,
12:False,
13:True,
14:False,
15:True,
16:False,
17:False,
18:False,
19:False,
20:False,
21:False,
22:False,
23:True,
24:False,
25:True,
26:False,
27:False,
28:False,
29:False,
31:False,
32:False,
33:False,
34:False,
35:False,
36:True,
37:False,
38:False,
39:False,
40:False,
41:False,
42:False,
43:False,
44:False,
45:False,
46:False
}


# Estimated cortical depth (µm) per cell based on Neuropixels channel position
DICT_CORT_DEPTH = {
1:843.9,
2:723.5,
3:1024.1,
4:836.7,
5:389.4,
6:1150.1,
7:1001.3,
8:1017.9,
10:718.0,
12:1005.6,
13:1030.4,
14:816.5,
15:620.1,
16:1412.8,
17:873.5,
18:1381.2,
19:1180.9,
20:948.5,
21:1754.0,
22:1529.4,
23:1315.6,
24:1133.7,
25:1006.2,
26:1112.1,
27:1456.1,
28:1372.4,
29:1706.5,
31:1479.5,
32:1363.4,
33:1312.8,
34:1454.4,
35:1271.2,
36:1480.7,
37:891.8,
38:1721.3,
39:1586.5,
40:1250.9,
41:1413.5,
42:1348.9,
43:1399.3,
44:1141.4,
45:1137.0,
46:1039.7
}

# Alias for DICT_EAP_WAV — same values, kept for backwards compatibility
DICT_CLEAR_EAP_WAV = {

1:False,
2:True,
3:False,
4:True,
5:False,
6:False,
7:True,
8:False,
10:False,
12:True,
13:False,
14:True,
15:True,
16:True,
17:False,
18:True,
19:True,
20:False,
21:True,
22:True,
23:False,
24:True,
25:False,
26:True,
27:True,
28:True,
29:True,
31:False,
32:False,
33:False,
34:False,
35:False,
36:False,
37:True,
38:False,
39:False,
40:False,
41:False,
42:True,
43:False,
44:True,
45:True,
46:True
}

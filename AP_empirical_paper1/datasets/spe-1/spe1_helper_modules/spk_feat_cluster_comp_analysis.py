...
def _compute_pct_within_per_cell(wf_dir, spike_fit_dir, half_win=75):
    ...
    # load per-feature between-cell thresholds
    feat_thresholds = {}
    ...
    for pkl in pkls:
        ...
        row = {
            'cell_id':   cid,
            'pct_nrmse': float(np.mean(nrmse_w > btw_mean_nrmse) * 100),
            'pct_cos':   float(np.mean(cos_w < btw_mean_cos) * 100),
        }
...
def plot_pct_within_vs_metadata(df_master, wf_dir, spike_fit_dir, half_win=75):
    ...
    # all metrics: whole-waveform first, then per-feature
    ...
    for col_name, metric_label in metrics:
        ...
        # Panel 6 — cortical depth scatter
        ax = axes[5]

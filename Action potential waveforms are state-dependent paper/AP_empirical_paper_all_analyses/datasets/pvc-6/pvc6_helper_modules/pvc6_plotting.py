...
def plot_avg_waveform_by_stim_type(all_constant_spks, all_ramp_spks, all_pink_spks,
                                    fs=200000, window_length=(10, 10), xlim=(-2, 2)):
    """Mean +/- SD of the smoothed voltage derivative (dV/dt) for each stimulus type.

    NOTE: the inputs are dV/dt (see process_sweeps: `np.diff` then LOWESS-smoothed),
    not raw voltage. During the fast phase of a spike, transmembrane current is
    capacitance-dominated (I ~ C * dV/dt), so this is commonly used as a rough proxy
    for what an extracellular electrode near the soma/axon would pick up (an inferred
    EAP), not the actual intracellular waveform -- see plot_avg_iap_by_stim_type for
    the real IAP. Pass None to skip a stimulus type.
    """
    plt.figure(figsize=(10, 6))
    one_ms = fs / 1000.0
    window_pre = one_ms * window_length[0]

    def _t_ms(n):
        # d_smoothed_data is np.diff(windowed_data), so index i corresponds to the
        # original window's sample (i + 1), and the window is peak-relative.
        return (np.arange(1, n + 1) - window_pre) / one_ms

    if all_constant_spks is not None:
        mean_c = np.mean(all_constant_spks, axis=0)
        std_c  = np.std(all_constant_spks, axis=0)
        t = _t_ms(len(mean_c))
        plt.plot(t, mean_c, label='Mean Constant Spikes', color='green', linewidth=4)
        plt.fill_between(t, mean_c - std_c, mean_c + std_c, color='green', alpha=0.3)

    if all_ramp_spks is not None:
        mean_r = np.mean(all_ramp_spks, axis=0)
        std_r  = np.std(all_ramp_spks, axis=0)
        t = _t_ms(len(mean_r))
        plt.plot(t, mean_r, label='Mean Ramp Spikes', color='purple', linewidth=4)
        plt.fill_between(t, mean_r - std_r, mean_r + std_r, color='purple', alpha=0.3)

    if all_pink_spks is not None:
        mean_p = np.mean(all_pink_spks, axis=0)
        std_p  = np.std(all_pink_spks, axis=0)
        t = _t_ms(len(mean_p))
        plt.plot(t, mean_p, label='Mean Pink Spikes', color='hotpink', linewidth=4)
        plt.fill_between(t, mean_p - std_p, mean_p + std_p, color='lightpink', alpha=0.3)

    plt.xlabel('Time from peak (ms)')
    plt.ylabel('Inferred EAP (a.u., smoothed dV/dt)')
    plt.title('Inferred extracellular-like signal (smoothed dV/dt), by stimulation type')
    plt.legend()
    plt.xlim(xlim)
    plt.show()


def plot_avg_iap_by_stim_type(all_constant_iap, all_ramp_iap, all_pink_iap,
                               fs=200000, window_length=(10, 10), xlim=(-2, 2)):
    """Mean +/- SD of the actual raw intracellular voltage (IAP) for each stimulus type.

    Companion to plot_avg_waveform_by_stim_type: same window/alignment convention,
    but for real voltage (mV) rather than the smoothed dV/dt proxy. Pass None to skip
    a stimulus type.
    """
    plt.figure(figsize=(10, 6))
    one_ms = fs / 1000.0
    ...
    def _t_ms(n):
        return (np.arange(n) - window_pre) / one_ms

    if all_constant_iap is not None:
        mean_c = np.mean(all_constant_iap, axis=0)
        std_c  = np.std(all_constant_iap, axis=0)
        t = _t_ms(len(mean_c))
        plt.plot(t, mean_c, label='Mean Constant Spikes', color='green', linewidth=4)
        plt.fill_between(t, mean_c - std_c, mean_c + std_c, color='green', alpha=0.3)

    if all_ramp_iap is not None:
        mean_r = np.mean(all_ramp_iap, axis=0)
        std_r  = np.std(all_ramp_iap, axis=0)
        t = _t_ms(len(mean_r))
        plt.plot(t, mean_r, label='Mean Ramp Spikes', color='purple', linewidth=4)
        plt.fill_between(t, mean_r - std_r, mean_r + std_r, color='purple', alpha=0.3)

    if all_pink_iap is not None:
        mean_p = np.mean(all_pink_iap, axis=0)
        std_p  = np.std(all_pink_iap, axis=0)
        t = _t_ms(len(mean_p))
        plt.plot(t, mean_p, label='Mean Pink Spikes', color='hotpink', linewidth=4)
        plt.fill_between(t, mean_p - std_p, mean_p + std_p, color='lightpink', alpha=0.3)
    ...
    plt.title('Actual intracellular action potential (IAP), by stimulation type')
    plt.legend()
    plt.xlim(xlim)
    plt.show()

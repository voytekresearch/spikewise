...
- Explain the substantial constant-to-pink misclassification. Test/report whether low-amplitude constant-current sweeps, which are closer to the average pink-noise amplitude, are preferentially classified as pink. Add the supporting analysis and statistics to the supplement. [Ryan 63]
  - Addressed as a short aside in Methods (PVC-6: STIMULUS TYPE DECODING), folded into the same paragraph as the out-of-fold procedure sentence (not a new figure, new Methods subsection, or main-text addition). Full paragraph addition (appended after the existing "...Feature importances were averaged across bootstrap iterations to estimate mean importance and variability per feature." sentence): *"For the random forest classifier specifically, the confusion matrix and feature importances reported in Fig. 3B-C use out-of-fold predictions instead: 5-fold cross-validation (same hyperparameters as above; `class_weight` unweighted) was repeated 1,000 times with independent random 5-fold partitions, yielding a stable accuracy estimate (mean ± SD) and an averaged confusion matrix; feature importances were pooled across all fold-level estimators from the same repeats. In this analysis, constant-current spikes were frequently misclassified as pink noise (Fig. 3B). This was more likely for sweeps with lower current, closer to the current level typically used for pink noise (Pearson r = -0.78, p < 0.0001, n = 20 sweeps), pointing to the importance of current amplitude in shaping AP waveform, consistent with the pink-noise-specific analysis below."* Full underlying analysis and plots exist in `spk_waveform_stim_predictors_cell1_main.ipynb` and `pvc6_supp_final_figures.ipynb` for anyone who wants to see them (also mention to Ryan directly in the reply), but aren't promoted to a paper figure. [Blanca]
- Report exact p-values for the pairwise correlations in Figure 3E and clarify/apply correction for multiple comparisons. [Ashley 65]
  - Addressed: `plot_top_correlations_by_window` in `pvc6_plotting.py` now computes all spike x stim pairwise correlations up front, applies BH-FDR (`statsmodels.stats.multitest.fdrcorrection`) across that full set of tests, prints an exact r/p/q table, and gates the displayed significance stars on the FDR-corrected q-value rather than raw p<0.05.
  - Also found while fixing this: panel G's caption claims the beta weights are FDR corrected, but `plot_beta_weights_combined` was only thresholding on raw p<0.05 — no correction was ever applied to those per-coefficient p-values. Fixed the same way (BH-FDR across all target x feature tests, filtered/starred on q). Re-run Figure 3 to confirm which beta weights remain significant after correction, since some may drop out. [Blanca]
...
- Update panel E’s statistical reporting with exact p-values and multiple-comparison handling. [Ashley 65] — addressed in `pvc6_plotting.py` (BH-FDR + exact values); also caught and fixed the same gap in panel G, whose caption claimed FDR correction that the code wasn't actually doing.
